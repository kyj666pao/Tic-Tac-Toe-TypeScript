...
init();
function init(): void {
  let allSqr = document.querySelectorAll(".sqr p") as NodeListOf<Element>;
...
function updateMessage(): void {
  winner == false && tie == false
    ? (messageEl.innerHTML = `Now is ${ 
        turn == 1 ? "Player2's (X)" : "Player1's (O)"
        } turn <br /> please select a square`)
    : winner == false && tie == true
    ? (messageEl.innerHTML = `Tie, Both you do a good job! 
        <br /> The Game Will Reset in 3 seconds`)
    : (messageEl.innerHTML = `${
        turn == 1 ? "Player1's (O)" : "Player2's (X)"
        } Win <br /> The Game Will Reset in 3 seconds`);
}
...
function play(evt: MouseEvent): void {
  ...
  let O = document.createElement("p") as HTMLParagraphElement;
  O.innerText = choice[0];
  O.setAttribute("class", "OO");
  let X = document.createElement("p") as HTMLParagraphElement;
  X.innerText = choice[1];
  X.setAttribute("class", "XX");
  let sqId: string = target.id;
  ...
  let allSqr = document.querySelectorAll<HTMLParagraphElement>(`#${sqId} p`);
  ...
  if (winner === true as boolean || tie === true as boolean) {
    setTimeout(() => {
      init();
    }, 3000);
    return;
...
function checkForWin(): void {
  if (
    (board[0] == board[1] && board[0] == board[2] && board[0] != 0) ||
